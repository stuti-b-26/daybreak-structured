...
const CARD_SVGS = [
  'assets/svg/decorative/hm-deco-text-container-01.svg',
  'assets/svg/decorative/hm-deco-text-container-02.svg',
  'assets/svg/decorative/hm-deco-text-container-03.svg'
];
const CARD_COLORS = ['adlibs__card--orange', 'adlibs__card--blue', 'adlibs__card--purple'];
const EXAMPLES = [
...
const COMM_CARDS = [
  ...
  { text:'"lol he has a little face"',                       author:'— anonymous', bg:'hm-deco-text-container-03.svg' }
];

const WORD_BANK = {
  'adjective':    ['soggy','chaotic','majestic','extremely confused','tiny','sparkly','cursed','distinguished'],
  'adjective 2':  ['deeply unsettling','aggressively wholesome','legally questionable','extremely moist','suspiciously calm'],
  'verb':         ['moonwalk','screech','vigorously nap','overthink','befriend','aggressively wave at','yodel at'],
  'place':        ["a Denny's parking lot",'the moon','my backyard','the discount bin','a haunted Costco','Cleveland','a Walmart'],
  'noun':         ['a spatula','a kazoo','an unread email','a very long receipt','a single sock','a coupon','a vibe'],
  'body part':    ['chin','left elbow','third toe','dominant eyebrow','kneecap','forehead'],
  'past verb':    ['sneezed','giggled','filed a formal complaint','moonwalked away','logged off','unfollowed me','sighed loudly'],
  'plural noun':  ['honks','spreadsheets','vibes','croutons','apologies','sneezes','red flags'],
  'plural noun 2':['feelings','napkins','notifications','mystery items','coupons','thoughts and prayers'],
  'animal':       ['golden retriever','dramatic llama','confused penguin','overcaffeinated squirrel','sad crab'],
  'number':       ['7','42','400','3000','17','99','2'],
  'number 2':     ['12','45','8','200','30','3'],
  'number 3':     ['6','2','15','100','4','1'],
  'job title':    ['accountant','freelance wizard','substitute teacher','professional napper','life coach','unsolicited advisor'],
  'authority':    ['HOA president','librarian','a golden retriever','the vibe police','Big Soup','my landlord'],
  'quote':        ['"it is what it is"','"we move"','"this is fine"','"not my problem"','"honestly fair"'],
};
...
/* topic label clicks */
document.querySelectorAll('.adlibs__tl:not(#randomTopic)').forEach(el => {
...
/* random topic — activates like a normal pill but doesn't reveal which topic */
document.getElementById('randomTopic').addEventListener('click', () => {
  curTopic = TOPICS[Math.floor(Math.random() * TOPICS.length)];
  document.querySelectorAll('.adlibs__tl').forEach(x => x.classList.remove('adlibs__tl--active'));
  document.getElementById('randomTopic').classList.add('adlibs__tl--active');
  buildForm();
});
document.getElementById('randomizeBtn').addEventListener('click', () => {
  document.querySelectorAll('#adlibForm input').forEach(inp => {
    const opts = WORD_BANK[inp.dataset.key];
    if (opts) inp.value = opts[Math.floor(Math.random() * opts.length)];
  });
  if (document.getElementById('storyResult').style.display !== 'none') revealStory();
});
...
function buildForm() {
  ...
  document.querySelector('#revealBtn .svgbtn__text').textContent = 'reveal story';
  ...
  curTopic.f.forEach((field, i) => {
    ...
    bg.src = i % 2 === 0
      ? 'assets/svg/decorative/hm-deco-text-field-container-01.svg'
      : 'assets/svg/decorative/hm-deco-text-field-container-02.svg';
    bg.alt = ''; bg.setAttribute('aria-hidden','true');

    const prefix = document.createElement('span');
    prefix.className = 'adlibs__input-prefix';
    prefix.textContent = field.k + ':';
    ...
    wrap.appendChild(bg); wrap.appendChild(prefix); wrap.appendChild(inp);
    row.appendChild(wrap);
    f.appendChild(row);
  });
  requestAnimationFrame(alignInspiration);
}
...
function revealStory() {
  ...
  document.querySelector('#revealBtn .svgbtn__text').textContent = 'hide story';
}
...
EXAMPLES.forEach((e, i) => {
  ...
  card.className = `adlibs__card ${CARD_COLORS[i % 3]}`;
...
const COMM_DRAWINGS = [
  'assets/svg/decorative/hm-deco-drawing-01.svg',
  'assets/svg/decorative/hm-deco-drawing-02.svg',
  'assets/svg/decorative/hm-deco-drawing-03.svg'
];
COMM_CARDS.forEach((c, i) => {
  const card = document.createElement('div');
  card.className = `community-card ${CARD_COLORS[i % 3]}`;
  const drwWrap = document.createElement('div');
  drwWrap.className = 'cc-drawing-wrap'; drwWrap.dataset.drw = `${i+1}`;
  const drw = document.createElement('img');
  drw.src = COMM_DRAWINGS[i]; drw.className = `deco-drw${i+1}`; drw.alt = ''; drw.setAttribute('aria-hidden','true');
  drwWrap.appendChild(drw);
  const inner = document.createElement('div');
  inner.className = 'cc-inner';
  inner.innerHTML = `<p class="cc-text">${c.text}</p><p class="cc-author">${c.author}</p>`;
  card.appendChild(drwWrap); card.appendChild(inner);
...
const NOTE_NAMES = ['C','B','A','G','F','E','D','C'];
const FREQS = {
  piano:[523,494,440,392,349,330,294,262],
  quack:[900,800,750,850,700,950,820,760],
  fart: [80, 70, 65, 60, 75, 68, 72, 58 ],
  boing:[400,350,300,450,380,420,360,320],
  meow: [600,550,500,650,480,520,560,440]
};
let grid=[], isPlaying=false, isLooping=true, playInterval=null, audioCtx=null, noteHist=[];
let curPack = 'quack';

/* sound pack buttons */
const BEAT_NAMES = ['hat','open','snare','clap','kick','tom','bass','perc'];
...
function playBeat(row) {
  ...
  if      (row===0) { noise(0.05,0.3,10000); }                          // closed hi-hat
  else if (row===1) { noise(0.22,0.25,8000); }                          // open hi-hat
  else if (row===2) { noise(0.15,0.5,1500); kick(200,150,0.08,0.2); }  // snare
  else if (row===3) { noise(0.08,0.9,2000); }                           // clap
  else if (row===4) { kick(160,0.01,0.45,1.0); }                        // kick
  else if (row===5) { kick(120,0.01,0.3,0.7); }                         // tom
  else if (row===6) { kick(80,0.01,0.5,0.9); }                          // bass
  else if (row===7) { noise(0.04,0.6,3000); kick(300,200,0.04,0.3); }  // perc/rim
}
...
document.getElementById('playBtn').addEventListener('click', () => {
  ...
  if (isPlaying) { clearInterval(playInterval); isPlaying=false; document.querySelector('#playBtn .svgbtn__text').textContent='play track'; btn.style.minWidth=''; return; }
  btn.style.minWidth = btn.offsetWidth + 'px';
  isPlaying=true; document.querySelector('#playBtn .svgbtn__text').textContent='stop';
  let col=0;
  playInterval=setInterval(()=>{ playCol(col); col++; if(col>=COLS){ if(isLooping){col=0;}else{clearInterval(playInterval);isPlaying=false;document.querySelector('#playBtn .svgbtn__text').textContent='play track';document.getElementById('playBtn').style.minWidth='';}}},210);
});
...
let balls=[],bDrag=null,bDox=0,bDoy=0,bMx=0,bMy=0,bLx=0,bLy=0,bVx=0,bVy=0,bW=0,bH=0,bFloor=0,bActive=false,bGrav=0.4;

function resizeBall(){bW=window.innerWidth;bH=window.innerHeight;updateBallFloor();}
function updateBallFloor(){const f=document.getElementById('site-footer');if(!f){bFloor=bH;return;}const t=f.getBoundingClientRect().top;bFloor=(t>0&&t<bH)?t:bH;}
function mkBall(){
  const r=22,color=BCOLS[Math.floor(Math.random()*BCOLS.length)];
  const el=document.createElement('div');
  el.className='ball-dot';
  el.style.cssText=`width:${r*2}px;height:${r*2}px;background:${color};`;
  document.body.appendChild(el);
  return{x:r+Math.random()*(bW-r*2),y:-r-Math.random()*300,r,vx:(Math.random()-.5)*4,vy:0,color,el};
}
function collide(){
  for(let pass=0;pass<3;pass++){
    for(let i=0;i<balls.length;i++)for(let j=i+1;j<balls.length;j++){
      const a=balls[i],b=balls[j],dx=b.x-a.x,dy=b.y-a.y,d=Math.sqrt(dx*dx+dy*dy),m=a.r+b.r;
      if(d<m&&d>.01){
        const nx=dx/d,ny=dy/d,ov=(m-d)/2;
        if(bDrag!==a){a.x-=nx*ov;a.y-=ny*ov;}
        if(bDrag!==b){b.x+=nx*ov;b.y+=ny*ov;}
        if(pass===0){const dv=(a.vx-b.vx)*nx+(a.vy-b.vy)*ny;if(dv>0){const im=dv*.7;if(bDrag!==a){a.vx-=im*nx;a.vy-=im*ny;}if(bDrag!==b){b.vx+=im*nx;b.vy+=im*ny;}}}
      }
    }
  }
}
function updateBalls(){for(const b of balls){if(b===bDrag)continue;b.vy+=bGrav;b.vx*=.993;b.vy*=.993;b.x+=b.vx;b.y+=b.vy;if(b.x-b.r<0){b.x=b.r;b.vx=Math.abs(b.vx)*.68;}if(b.x+b.r>bW){b.x=bW-b.r;b.vx=-Math.abs(b.vx)*.68;}if(b.y+b.r>bFloor){b.y=bFloor-b.r;b.vy=-Math.abs(b.vy)*.68;b.vx*=.85;}if(b.y-b.r<0){b.y=b.r;b.vy=Math.abs(b.vy)*.68;}}collide();if(bDrag){bDrag.x=bMx-bDox;bDrag.y=bMy-bDoy;bDrag.vx=bVx*1.3;bDrag.vy=bVy*1.3;}bVx=bVx*.5+(bMx-bLx)*.5;bVy=bVy*.5+(bMy-bLy)*.5;bLx=bMx;bLy=bMy;}
function drawBalls(){for(const b of balls)b.el.style.transform=`translate(${b.x-b.r}px,${b.y-b.r}px)`;}
function bloop(){if(!bActive)return;updateBallFloor();drawBalls();updateBalls();requestAnimationFrame(bloop);}
...
document.addEventListener('touchmove',e=>{if(!bActive)return;bLx=bMx;bLy=bMy;bMx=e.touches[0].clientX;bMy=e.touches[0].clientY;if(bDrag)e.preventDefault();},{passive:false});
document.addEventListener('touchend',()=>{bDrag=null;});

document.getElementById('addBallsBtn').addEventListener('click',()=>{
  if(!bActive){bActive=true;resizeBall();bloop();setupMotionSensors();}
...
const GRAV_OPTS=[{v:0.4,label:'normal'},{v:0.1,label:'moon'},{v:1.4,label:'heavy'},{v:0,label:'zero-g'},{v:-0.15,label:'inverted'}];
let gravIdx=0;
document.getElementById('gravBtn').addEventListener('click',()=>{gravIdx=(gravIdx+1)%GRAV_OPTS.length;bGrav=GRAV_OPTS[gravIdx].v;document.getElementById('gravBtnText').textContent=GRAV_OPTS[gravIdx].label;});

/* Device motion shake (mobile only) */
let motionSetup=false,lastAccX=0,lastAccY=0;
function onDeviceMotion(e){
  if(!bActive||!balls.length)return;
  const acc=e.accelerationIncludingGravity;if(!acc)return;
  const ax=acc.x||0,ay=acc.y||0;
  const jerk=Math.abs(ax-lastAccX)+Math.abs(ay-lastAccY);
  if(jerk>22){
    const ix=(ax-lastAccX)*0.6,iy=-(ay-lastAccY)*0.6;
    for(const b of balls){b.vx+=ix+(Math.random()-.5)*3;b.vy+=iy+(Math.random()-.5)*3;}
  }
  lastAccX=ax;lastAccY=ay;
}
function setupMotionSensors(){
  if(motionSetup)return;motionSetup=true;
  if(typeof DeviceMotionEvent==='undefined')return;
  const req=typeof DeviceMotionEvent.requestPermission==='function'
    ?DeviceMotionEvent.requestPermission():Promise.resolve('granted');
  req.then(s=>{if(s==='granted')window.addEventListener('devicemotion',onDeviceMotion);}).catch(()=>{});
}
...
const discoBallImg=new Image();discoBallImg.src='assets/svg/decorative/hm-deco-disco-ball.svg';
...
/* ============================================
   ADLIBS LAYOUT HELPERS
   ============================================ */
function alignInspiration() {
  const right = document.querySelector('.adlibs__right');
  if (!right) return;
  if (window.innerWidth < 1200) { right.style.paddingTop = ''; return; }
  const layout = document.querySelector('.adlibs__layout');
  const pills  = document.querySelector('.adlibs__topic-labels');
  if (!layout || !pills) return;
  const layoutTop = layout.getBoundingClientRect().top;
  const pillsTop  = pills.getBoundingClientRect().top;
  right.style.paddingTop = Math.max(0, pillsTop - layoutTop) + 'px';
}

function fitCardText() {
  document.querySelectorAll('.adlibs__card').forEach(card => {
    card.style.width = '';
    card.style.minWidth = '';
